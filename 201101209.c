 ...
 * 
 * <Name : Vaibhavi Desai; Email id : 201101209@daiict.ac.in>
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
void waitfg(pid_t pid)
{
	...
	struct job_t *s5;
	s5 = getjobpid(jobs,pid);
	while(s5!=NULL&&(s5->state==FG))//we need to wait for the FG process to finish
    	{
      	sleep(1);
	...
	pid_t pid;
		...
		while((pid=waitpid(-1,&status,WNOHANG|WUNTRACED))>0)
		{
			...
			if(WIFSTOPPED(status))
			{
				...
				s4->state = 3; //changing the state of stopped jobs to ST i.e. 3
			}
